...
# Load the Data
prosperLoan <- read.csv(file = "D:\\Users\\wangsb\\Downloads\\prosperLoanData.csv", header = TRUE, sep = ",")
...
p1 <- qplot(data = subset(prosperLoan, CreditGrade > 0), x = ListingCreationDate, bins = 30) + ylim(0,10000)
p2 <- qplot(data = subset(prosperLoan, ProsperRating > 0), x = ListingCreationDate, bins = 90)
...
prosperLoan$CreditRating[which(prosperLoan$ProsperRating > 0)] <- prosperLoan$ProsperRating[which(prosperLoan$ProsperRating > 0)]

prosperLoan$TermYears <- as.numeric(levels(prosperLoan$Term))[prosperLoan$Term] / 12
```
...
```{r echo=FALSE}
cor.test(~ BorrowerRate + ProsperScore, data = subset(prosperLoan, ProsperScore > 0 & ProsperScore < quantile(ProsperScore, 0.99)))
```
...
```{r echo=FALSE}
cor.test(~ CreditRating + ProsperScore, data = subset(prosperLoan, ProsperScore > 0 & ProsperScore < quantile(ProsperScore, 0.99)))
```
...
ggplot(data = subset(prosperLoan, !is.na(IncomeRange) & LoanOriginalAmount > 0), aes(x = LoanOriginalAmount, fill = IncomeRange)) +
  geom_density() +  
  xlab("Amount in Dollars") + 
  scale_x_continuous(breaks = seq(0,35000,10000)) +
  facet_wrap(~IncomeRange, ncol = 3) +
  coord_cartesian(xlim = c(0, quantile(prosperLoan$LoanOriginalAmount, 0.99, na.rm = TRUE))) +
...
```{r echo=FALSE}
cor.test(~ BorrowerRate + LoanOriginalAmount, data = subset(prosperLoan, LoanOriginalAmount > 0 & LoanOriginalAmount < quantile(LoanOriginalAmount, 0.99)))
```
...
```{r echo=FALSE}
ggplot(data = subset(prosperLoan, ProsperScore > 0 & Term == '36'), aes(ListingCreationDate, BorrowerRate)) +
  geom_point(aes(color = ProsperScore), alpha = 0.2) + 
  scale_color_continuous(high = "blue", low = "green")
```
